...
class TestHubUpdateBuyingEventBom:

    def test_update_buying_event_bom_success(self):
        test_parent_opp = 'ODI-TEST'
        test_cscore = 0
        test_tcv_value = -1000
        test_salesstage_cd = '1'
        result = hub_parent_opp_update.update_buying_event_bom(test_parent_opp, test_cscore,
                                                         test_tcv_value, test_salesstage_cd)
        print(f"Result: {result}")
        assert result is not None

    def test_update_buying_event_bom_fail(self):
        test_parent_opp = 'ZZZZZ'
        test_cscore = 80
        test_tcv_value = 35000
        test_salesstage_cd = '7'
        # negative test with invalid parent opp
        result = hub_parent_opp_update.update_buying_event_bom(root_opportunity=test_parent_opp,
